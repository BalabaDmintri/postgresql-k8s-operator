...
async def test_scaling_to_zero(ops_test: OpsTest, continuous_writes) -> None:
    ...
    original_leader_unit = await get_primary(ops_test, database_app_name=APP_NAME)
    logger.info(f"------------ original ---1 {original_leader_unit}")
    second_leader_unit = await get_primary(ops_test, database_app_name=SECOND_CLUSTER_APP_NAME)
    logger.info(f"------------ original ---{second_leader_unit}")
    # original_host_path, original_pv_id = get_host_path(original_leader_unit)
    # logger.info("------------ original ---3")
    # logger.info(f"----------- original_pv_id = {original_pv_id}")
    # logger.info(f"----------- original_host_path = {original_host_path}")
    # second_host_path, second_pv_id = get_host_path(second_leader_unit)
